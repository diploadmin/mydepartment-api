...
class Neo4jGraphClient:
    ...
    async def get_document_by_slug(
        ...
        """Fuzzy URL lookup: extract the slug and match via CONTAINS.

        Handles the common case where Weaviate and Neo4j store different
        URL variants for the same page (e.g. /blog/my-post/ vs /page/my-post/).
        Falls back to a shorter prefix of the slug for near-miss slugs.
        """
        await self.connect()
        ...
        query = """
        MATCH (d:Document)
        WHERE d.url CONTAINS $slug AND d.url CONTAINS $domain
        RETURN d, labels(d) AS labels
        LIMIT 1
        """
        async with self._driver.session(database=database) as session:
            result = await session.run(query, slug=slug, domain=domain)
            record = await result.single()

            if not record and len(slug) > 20:
                short_slug = slug[:min(40, len(slug) * 2 // 3)]
                result = await session.run(query, slug=short_slug, domain=domain)
                record = await result.single()
    ...
    async def get_entity_relations(
        self, entity_names: list[str], database: str, limit_per_entity: int = 15
    ) -> dict[str, list[GraphRelation]]:
        """Outgoing relations from non-Document entities (Topic, Person, Actor, etc.) by name."""
        if not entity_names:
            return {}
        await self.connect()
        query = """
        UNWIND $names AS ename
        MATCH (e {name: ename})-[r]-(other)
        WHERE e.name <> other.name
        RETURN e.name                    AS source_name,
               ''                        AS source_hash,
               type(r)                   AS rel_type,
               other.name               AS target_name,
               coalesce(other.document_hash, '') AS target_hash,
               coalesce(other.url, '')   AS target_url,
               coalesce(other.post_type, '') AS target_post_type,
        ...
        try:
            async with self._driver.session(database=database) as session:
                ...
                records = [r async for r in result]
            by_entity: dict[str, list[GraphRelation]] = {}
            for r in records:
                rel = _record_to_relation(r)
                by_entity.setdefault(rel.source_name, []).append(rel)
            return by_entity
        except Exception as e:
            logger.warning("get_entity_relations failed: %s", e)
            return {}
...
_PROCESS_LABELS = {"Process", "Trend", "Technology"}
...
def _aggregate_relations(
    ...
    """Classify relations into topics, people, actors, countries, dates, processes, etc."""
    doc_name = ""
    ...
    for rel in direct:
        ...
        if "Topic" in rel.target_labels or "TopicBasket" in rel.target_labels:
            ...
            if tname and tname not in seen_names:
                actors.append(tname)
                seen_names.add(tname)
        elif "Country" in rel.target_labels:
            if tname and tname not in seen_names:
                countries.append(tname)
                seen_names.add(tname)
        elif "Date" in rel.target_labels:
            if tname and not date_published:
                date_published = tname
        elif tl & _PROCESS_LABELS:
            if tname and tname not in seen_names:
                processes.append(tname)
                seen_names.add(tname)
